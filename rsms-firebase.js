...
// ── FIREBASE CONFIG — loaded from rsms-config.js ────────────────
// Config is in rsms-config.js which is NOT committed to GitHub
// Upload rsms-config.js manually to your hosting server
var SHARED_CONFIG = (window.RSMS_CONFIG && window.RSMS_CONFIG.firebase) || null;
...
window.RSMS_DB = {
  ...
  init: function(onReady){
    ...
    if(!SHARED_CONFIG){
      console.warn('RSMS_DB: rsms-config.js not loaded — offline only'); _ready=true; _readyCbs.forEach(function(cb){cb();}); return;
    }
    if(!_schoolId){
