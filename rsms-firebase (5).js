...
var RSMS_FB = (function(){
  ...
  function saveScores(cls, term, sess, data){
    var lsKey = _scoreKey(cls,term,sess);
  ...
  function saveClockLogs(data){
    localStorage.setItem('rsms_clock_logs', JSON.stringify(data));
  ...
  function saveLogo(logoDataUrl){
    ...
    // Also update school info
    var sc = JSON.parse(localStorage.getItem('rsms_school')||'{}');
    sc.logoUrl = logoDataUrl;
    localStorage.setItem('rsms_school', JSON.stringify(sc));
  ...
  function saveSchool(data){
    ...
    // Include logo in school info saved to Firebase
  ...
  function warmCache(schoolId, onComplete){
    ...
    var keys = ['students','staff','fees','ct_remarks',
                'fee_schedule','stream_config','broadcasts','info','clock_logs','clock_cfg'];
    var done = 0;
    ...
    keys.forEach(function(key){
      _db.ref('schools/'+_sid+'/'+key).once('value').then(function(snap){
        ...
        if(val!==null){
          ...
          localStorage.setItem(lsKey, JSON.stringify(data));
          // Also restore logo from school info
          if(key==='info' && val && val.logoUrl){
            localStorage.setItem('rsms_school_logo', val.logoUrl);
          }
        }
        tick();
      }).catch(function(){ tick(); });
